...
async function initClient() {
    try {
        ...
        // Подписываемся на события входа/выхода
        ...
        document.getElementById('buttonDiv').onclick = handleAuthClick;
        document.getElementById('signout_button').onclick = handleSignoutClick;
        
        gapiInited = true;
        console.log('GAPI успешно инициализирован, isSignedIn:', isSignedIn);
    } catch (error) {
...
function handleSubmit(event) {
    ...
    if (isSignedIn) {
        ...
        handleAuthClick(); // Запускаем процесс авторизации
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const forms = document.querySelectorAll('form#diaryForm');
    forms.forEach(form => {
        if (form) {
            // Удаляем встроенный атрибут onsubmit, если он есть
